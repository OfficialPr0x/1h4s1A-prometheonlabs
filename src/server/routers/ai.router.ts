 ...
 * @function {({ type: string, content: string }) => Promise<{ widget: object }>} generateWidget - Generate a widget based on the type and content
 * @function {({ type: string, content: string }) => Promise<{ plugin: object }>} generatePlugin - Generate a plugin based on the type and content
 * @function {({ file: File }) => Promise<{ analysis: string, suggestions: string[] }>} analyzeFile - Analyze an uploaded file and provide improvement suggestions
...
export const AiRouter = Trpc.createRouter({
  ...
  generateWidget: Trpc.procedure
    ...
    .mutation(async ({ input }) => {
      ...
      const widget = await OpenaiService.generateWidget(
        input.type,
        input.content,
      )
  ...
  generatePlugin: Trpc.procedure
    ...
    .mutation(async ({ input }) => {
      ...
      const plugin = await OpenaiService.generatePlugin(
        input.type,
        input.content,
      )
  ...
  analyzeFile: Trpc.procedure
    ...
    .mutation(async ({ input }) => {
      ...
      const uploadedFile = await UploadService.uploadPrivate(file)

      const { analysis, suggestions } = await OpenaiService.analyzeFile(
        uploadedFile[0].url,
      )
